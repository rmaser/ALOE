...
def batch_norm_uncentered_2d(
    ...
    #rstd = torch.reciprocal(torch.sqrt(var + eps))[None, ..., None, None]
    rstd = torch.rsqrt(var + eps)[None, ..., None, None]
        ...
        if self.training and self.track_running_stats:
            ...
            if self.num_batches_tracked is not None:  # type: ignore[has-type]
                ...
                if self.momentum is None:  # use cumulative moving average
                    exponential_average_factor = torch.reciprocal(torch.tensor(float(self.num_batches_tracked))).item()
