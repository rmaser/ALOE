...
class NormedConv2d(nn.Conv2d):
    ...
    def forward(self, in_tensor: Tensor) -> Tensor:
        weight_norm = LA.vector_norm(self.weight, dim=(1, 2, 3), keepdim=True)
        weight_norm_inv = torch.reciprocal(weight_norm + 1e-12)
        w = self.weight * weight_norm_inv
    ...
    **kwargs : Any
    ...
    def __init__(
        ...
        **kwargs,  # bias is always False
        ...
        self.device = device
        self.dtype = dtype
        self.bias = None  # Set to None to match nn.Conv2d interface when bias=False
        ...
        # Convert kernel_size to tuple if it's an int (same as nn.Conv2d)
        if isinstance(kernel_size, int):
            kernel_size = (kernel_size, kernel_size)
        
        self.patch_size = int(np.prod(kernel_size))

        # Define the weight parameter directly (no nn.Conv2d wrapper)
    ...
    def reset_parameters(self) -> None:
        ...
        # Use standard initialization (same as nn.Conv2d)
...
class BcosConv2d_unnormed(BcosConv2d):
    """BcosConv2d without weight normalization."""
...
BcosUnnormedConv2d = BcosConv2d_unnormed
